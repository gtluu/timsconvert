...
def parse_parent_scan(scan, method_params, centroided=False):
    ...
    scan_dict = {'scan_number': list(set(scan['frame_indices'].values.tolist()))[0],
...
def parse_product_scans(scans, method_params, centroided=False):
    low_mz_values = scans['quad_low_mz_values'].unique().tolist()
    high_mz_values = scans['quad_high_mz_values'].unique().tolist()

    quad_mz_values = zip(low_mz_values, high_mz_values)
    precursor_values_list = [np.mean(i) for i in quad_mz_values]
    print(scans)
...
def get_ms1_frame_indices(raw_data, num_of_frames):
    ...
    for frame_num in range(1, num_of_frames):
        ...
        if low_mz_series.size == 1 and high_mz_series.size == 1:
            if ms1_quad_mz == low_mz_series and ms1_quad_mz == high_mz_series:
                ms1_frame_indices.append(frame_num)

    return ms1_frame_indices
...
def parse_raw_data(raw_data, input_filename, output_filename):
    ...
    ms1_indices = get_ms1_frame_indices(raw_data, num_of_frames)

    parent_scan_dict = {}
    product_scan_dicts = []
    #for i in range(len(ms1_indices)):
    for i in range(1):
        parent_scan = raw_data[ms1_indices[i]:ms1_indices[i] + 1].sort_values(by='mz_values')
        parent_scan_dict = parse_parent_scan(parent_scan, method_params)

        if i != len(ms1_indices) - 1:
            product_scans = raw_data[ms1_indices[i] + 1:ms1_indices[i + 1]].sort_values(by='quad_low_mz_values')
        elif i == len(ms1_indices) - 1:
            product_scans = raw_data[ms1_indices[i] + 1:].sort_values(by='quad_low_mz_values')
        product_scan_dicts.append(parse_product_scans(product_scans, method_params))

...
def parse_parent_scan(scan, method_params, centroided=False):
    ...
    return {'scan_number': list(set(scan['frame_indices'].values.tolist()))[0],
...
def parse_product_scans(scans, method_params, centroided=False):
    ...
    print(low_mz_values, high_mz_values)
...
def parse_raw_data(raw_data, input_filename, output_filename):
    ...
    for frame_num in range(1, num_of_frames+1, method_params['num_ramps_per_cycle']):
        ...
        if low_mz_series.size == 1 and high_mz_series.size == 1:
            if ms1_quad_mz == low_mz_series and ms1_quad_mz == high_mz_series:
                ...
                product_scans = raw_data[frame_num + 1: frame_num + method_params['num_ramps_per_cycle'] + 1]
                product_scans = product_scans.sort_values(by='quad_low_mz_values')
                product_scan_dicts = parse_product_scans(product_scans, method_params)
